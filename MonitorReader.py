...
monitorPath = r'C:\Users\michael\Desktop\RISCV_Investigacion\Python Project\Monitor\\'
resultsPath = r'C:\Users\michael\Desktop\RISCV_Investigacion\Python Project\Mibench results\\'
...
def __monitorInterpreter(lines):    
    ...
    for line in lines:
        ...
        elif "END" in line and injectionFlag:
            repeatingFlag = True
        ...
        #Classification Accumulator
            ...
            if "FC 00" in line:    #Correctable Not Essential
                CNEerror += 1
            elif "FC 20" in line:  #Uncorrectable Not essential
                UNEerror += 1
...
def __resultKeyComparator(key, lines):
    ...
    ERRper = 100* ((EC / totalLine))
    SuccessPer = 100* ((Match / totalLine))
    FatalitiesPer = 100*(FE/totalLine)

    #print("Out of total of {} lines, {} were successful.".format(totalLine, Match))
    #print("The Rates of Success: {}%".format(round(SuccessPer,3)))
    #print("Number of Successful Program Executions: {}".format(PC))
    #print("Number of Errors caused: {}".format(EC))
    #print("The Rate of Errors were: {}%".format(round(ERRper,3)))
    #print("Number of Times those Errors affected output chars: {} ".format(FE))
    #print("The Rate of Fatalities were: {}%".format(round(FatalitiesPer,3)))

    if (PC == 0 and EC == 0 and FE == 0):
        return([0,0,0,0,0])
    else:
        
        resultList = [PC,EC,FE, Match, totalLine]
        return(resultList)
...
def __resultCompilation(compLists):
    ...
    FE = sumList[2]
    Match = sumList[3]
    totalLines = sumList[4]

    ERRper = 100*((EC / Match))
    SuccessPer = 100* ((Match / totalLines))
    FatalitiesPer = 100*(FE/totalLines)

    Border("Compilation Report", '*')
    print("Out of total of {} lines, {} were successful.".format(totalLines, Match))
    print("The Rates of Success: {}%".format(round(SuccessPer,5)))
    print("Number of Successful Program Executions: {}".format(PC))
    print("Number of Errors caused: {}".format(EC))
    print("The Rate of Errors were: {}%".format(round(ERRper,5)))
    print("Number of Times those Errors affected output chars: {} ".format(FE))
    print("The Rate of Fatalities were: {}%".format(round(FatalitiesPer,5)))
